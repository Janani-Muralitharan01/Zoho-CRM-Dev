...
const NavBar = () => {
  ...
  return (
    <div className="navbar ">
      ...
      <Button label="Save"  style={{ height: '36px'}}/></span>
      <OverlayPanel
        ref={op}
        style={{ width: '450px' }}
      ...
      <OverlayPanel ref={setting}>
        <div onClick={handlerSiebarOptionOne} className='options'>Rename</div>
        <br />
        <div className='options' onClick={handlerSiebarOptionTwo}>Module Permisson</div>
      </OverlayPanel>
      <Sidebar
      ...
      ><div>
        <span className='optionSideBarHeader'>Layout Name</span><br/>
        <span><InputText
            type="text"
            className='border-noround ml-2 w-6'
            placeholder="Standard"
          /> <i className="pi pi-clone ml-2 mt-2"></i></span>
           <span className="flex justify-content-end btnEndStyle">
           <button className="buttonStyle font-normal" onClick={()=> setSidebar(!sidebar) } > Cancel </button>
     ...
     >
      <div>
        ...
        <p>Select the profile(s) that should have access to the module.</p>
                <MultiSelect className='w-12' value={selectedCities2} options={Options} onChange={(e) => setSelectedCities2(e.value)} optionLabel="name"  display="chip" />
                <span className="flex justify-content-end btnEndStyle">
           <button className="buttonStyle font-normal" onClick={()=> setSidebarModule(!sidebarModule) } > Cancel </button>
            <Button label="Save"  style={{ height: '36px'}}/>
          </span>
      </div>
